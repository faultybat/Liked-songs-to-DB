...
def connect_to_database():
    try:
        db_conn = sqlite3.connect("liked_Songs.db")
        return db_conn
    except:
...
def insert_songlist(songlist):
    for x in range(0, len(songlist)):
        print(x)
        insert_track(songlist[x])
        insert_artist(songlist[x])
        insert_album(songlist[x])
        insert_linker(songlist[x])


def check_missing_song(songlist):
    db_conn = connect_to_database()
    cursor = db_conn.cursor()
...
def mark_deleted(deleted_song_id):
    db_conn = connect_to_database()
    cursor = db_conn.cursor()
